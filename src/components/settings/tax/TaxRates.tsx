...
const TaxRates = () => {
  ...
  const fetchTaxRates = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${API_URL}/api/dashboard/${organizationId}/settings/tax`);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(`${API_URL}/api/dashboard/${organizationId}/settings/tax/${taxRateToDelete.id}`);
      setTaxRates(prev => prev.filter(rate => rate.id !== taxRateToDelete.id));
  ...
  const handleSave = async () => {
    if (editingTaxRate) {
      ...
      // Handle new tax rate
      if (newTaxRate.tax_name && newTaxRate.tax_type && newTaxRate.tax_rate !== undefined) {
        try {
        ...
        <div className="space-y-4 border rounded-md p-4">
          ...
          {taxRates.map((rate) => (
            <div key={rate.id} className="grid grid-cols-5 gap-4 items-center">
              <div>{rate.tax_name}</div>
              <div>{rate.tax_type}</div>
              <div>{rate.tax_rate}%</div>
              <div>{rate.tax_description}</div>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => {
                  if (editingTaxRate) {
                    setEditingTaxRate(prev => ({ ...prev!, tax_name: e.target.value }));
                  } else {
                    setNewTaxRate(prev => ({ ...prev, tax_name: e.target.value }));
                  }
                }}
              />
            </div>
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => {
                  if (editingTaxRate) {
                    setEditingTaxRate(prev => ({ ...prev!, tax_type: value }));
                  } else {
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => {
                  if (editingTaxRate) {
                    setEditingTaxRate(prev => ({ ...prev!, tax_rate: parseFloat(e.target.value) }));
                  } else {
                    setNewTaxRate(prev => ({ ...prev, tax_rate: parseFloat(e.target.value) }));
                  }
                }}
              />
            </div>
            <div className="space-y-2">
              ...
              <Textarea
                ...
                value={editingTaxRate?.tax_description || newTaxRate.tax_description}
                onChange={(e) => {
                  if (editingTaxRate) {
                    setEditingTaxRate(prev => ({ ...prev!, tax_description: e.target.value }));
                  } else {
                    setNewTaxRate(prev => ({ ...prev, tax_description: e.target.value }));
                  }
                }}
              />
            </div>
